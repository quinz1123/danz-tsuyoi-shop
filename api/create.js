...
const PTLA="ptla_HDoMsQBrkjxBYtS0ei6h4pV7NwBHPxZfqgmQBMeczbv"   // <<< WAJIB GANTI
...
if(!user.attributes){
return res.json({error:user.errors?.[0]?.detail || "Gagal create user"})
}
...
return res.json({error:server.errors?.[0]?.detail || "Gagal create server"})
}
